...
# -*- coding: utf-8 -*-
"""
Finnish Parliament Speech Analysis Tool

This script analyzes sentiment and topics in Finnish parliamentary speech data.
It's specifically designed to process Finnish language transcripts from the Eduskunta (Finnish Parliament),
extract sentiment (positive/negative/neutral) from speeches, and optionally identify key topics.
The tool is optimized for different hardware environments (Apple Silicon, NVIDIA GPUs, or CPU-only)
and outputs results in both CSV and Markdown formats.
...
AVAILABLE_OLLAMA_MODELS: List[str] = [
    "gemma3:4b-it-qat",      # Default model (kept as first in the list)
    "gemma3:12b-it-qat",     
    "gemma3:27b-it-qat",     
    "llama3.1:8b",          
    "llama3.2:3b",          
...
# CSV_FILE_PATH: Path = Path(__file__).parent / "data" / "LLM_test_dataset.csv" # Original
# Changed for Finnish data, though CLI arg will take precedence for actual runs
# This DEFAULT_CSV_FILE_PATH is used if --csv_file is not provided.
# The actual input file will be specified via CLI: Parlament/sentiment_analyzer/raw_data/Finnish-1-1.csv

OUTPUT_BASE_DIR: Path = Path(__file__).parent / "output" 
OUTPUT_BASE_DIR.mkdir(parents=True, exist_ok=True)

# OUTPUT_MD_FILE_PATH and OUTPUT_CSV_FILE_PATH are now dynamically generated in main()
...
DEFAULT_SENTIMENT_MODEL = "gemma3:4b-it-qat"
DEFAULT_TOPIC_MODEL = "gemma3:4b-it-qat"
DEFAULT_CSV_FILE_PATH = "Parlament/sentiment_analyzer/raw_data/Finnish-1-1.csv" # MODIFIED for Finnish data
DEFAULT_OUTPUT_DIR = "Parlament/sentiment_analyzer/parlament_analysis/results_finnish" # MODIFIED output dir

# Column names in the input CSV
TEXT_COLUMN_SENTIMENT = "text" # MODIFIED for Finnish data (was "Sentence")
TEXT_COLUMN_TOPIC = "context" # MODIFIED for Finnish data (was "Sentence_longer")
GROUND_TRUTH_COLUMN = "NOT_APPLICABLE_Ground_Truth" # MODIFIED as Finnish data has no ground truth
DOC_ID_COLUMN = "ID" # MODIFIED for Finnish data (was "Doc ID")

# Add other original columns that should be carried over if they exist
ORIGINAL_COLUMNS_TO_CARRY = ["ID", "position", "context", "text"] # MODIFIED for Finnish data

# --- Helper Functions ---

# This construct_prompt is the complex one, not directly used by process_item's main flow anymore,
# but kept to minimize deletions from original script. process_item uses construct_sentiment_prompt and construct_topic_prompt.
...
def process_item(
    ...
    row_idx = item_input["row_idx"] 
    
    current_row_results = {**item_input} 
    current_row_results.update({
        "Row_Internal_Index": row_idx, 
        "Sentence_Snippet_Generated": " ".join(item_input.get("text", "").split()[:10]) + ("..." if len(item_input.get("text", "").split()) > 10 else ""),
    ...
    ground_truth = item_input.get(GROUND_TRUTH_COLUMN, "") # Will be "" if column is "NOT_APPLICABLE_..."
    ...
    if run_sentiment and sentiment_model_name:
        if not sentence_for_sentiment or sentence_for_sentiment == "N/A":
            print(f"  Skipping sentiment for row {row_idx} due to missing sentence ('{TEXT_COLUMN_SENTIMENT}' column).")
            ...
            api_response_text, error = call_ollama_api(sentiment_prompt_text, sentiment_model_name, actual_ollama_api_url, CLASSIFICATION_TEMPERATURE)
            ...
            if current_row_results[sentiment_col_name] not in ["Error_API", "Error_NoResponse", "N/A_MISSING_INPUT"]:
                # Ground truth is expected to be potentially missing or N/A for this dataset
    ...
    if run_topic and topic_model_name:
        if not context_for_topic or context_for_topic == "N/A":
            print(f"  Skipping topic for row {row_idx} due to missing context ('{TEXT_COLUMN_TOPIC}' column).")
            ...
            api_response_text, error = call_ollama_api(topic_prompt_text, topic_model_name, actual_ollama_api_url, CLASSIFICATION_TEMPERATURE)
...
def calculate_metrics_manually(results_list: List[Dict[str, Any]], model_column_name: str, ground_truth_column_key: str = GROUND_TRUTH_COLUMN): #MODIFIED to use constant
    ...
    # For Finnish data, ground_truth_column_key might point to a non-existent or empty column.
    # The logic below handles empty strings or "N/A" as references.
    ...
    if not valid_indices:
        print("  No valid predictions with corresponding ground truth found to calculate metrics.")
        return {"model": model_column_name, "accuracy": "N/A", "valid_predictions": 0, "correct_predictions": 0, "class_metrics": {}}
    ...
    classes = ["POSITIVE", "NEUTRAL", "NEGATIVE"] # Assuming these are the possible sentiment classes
    class_metrics = {}
    ...
    return {
        ...
        "accuracy": accuracy if valid_count > 0 else "N/A", # Return "N/A" if no valid ground truth
...
def preprocess_data(csv_file_path_str: str, rows_to_process_limit: int, cli_args: argparse.Namespace):
    ...
    with open(csv_file, 'r', encoding='utf-8') as f_csv:
        ...
        # --- MODIFIED: Required columns check for Finnish data ---
        required_cols_present = set(reader.fieldnames)
        missing_cols_error_list = []

        if cli_args.run_sentiment:
            if TEXT_COLUMN_SENTIMENT not in required_cols_present: 
                missing_cols_error_list.append(TEXT_COLUMN_SENTIMENT)
            if DOC_ID_COLUMN not in required_cols_present: 
                missing_cols_error_list.append(DOC_ID_COLUMN)
        
        if cli_args.run_topic:
            if TEXT_COLUMN_TOPIC not in required_cols_present: 
                missing_cols_error_list.append(TEXT_COLUMN_TOPIC)
            # Avoid adding DOC_ID_COLUMN twice to error if already added by sentiment check
            if DOC_ID_COLUMN not in required_cols_present and DOC_ID_COLUMN not in missing_cols_error_list:
                 missing_cols_error_list.append(DOC_ID_COLUMN)
        
        missing_cols_error_list = sorted(list(set(missing_cols_error_list))) # Unique and sorted

        if missing_cols_error_list:
            print(f"Error: CSV file {csv_file} must contain columns: {', '.join(missing_cols_error_list)} for the selected operations.")
            return []
        # --- End MODIFIED required columns check ---
            
        for i, row_dict in enumerate(reader):
            item = {"row_idx": i + 1} 
            for col_name in reader.fieldnames: 
                item[col_name] = row_dict.get(col_name, "").strip()
            all_items_read_from_csv.append(item)
            
    if not all_items_read_from_csv:
        print(f"No data rows found in {csv_file} after reading.")
        return []

    # --- MODIFIED: Filter based on validity for selected analyses (no Target_country) ---
    validated_items_for_processing = []
    for item in all_items_read_from_csv:
        # TEXT_COLUMN_SENTIMENT is now "text", TEXT_COLUMN_TOPIC is "context", DOC_ID_COLUMN is "ID"
        sentence_val = item.get(TEXT_COLUMN_SENTIMENT, "N/A").strip() 
        context_val = item.get(TEXT_COLUMN_TOPIC, "N/A").strip()      
        doc_id_val = item.get(DOC_ID_COLUMN, str(item["row_idx"])).strip() 

        # Determine if the item can be processed for *any* requested analysis
        can_be_processed = False
        if cli_args.run_sentiment and sentence_val not in ["", "N/A"]:
            can_be_processed = True
        if cli_args.run_topic and context_val not in ["", "N/A"]:
            can_be_processed = True
        
        # If neither sentiment nor topic is requested, or if both are requested but this item has no data for either
        analyses_requested = cli_args.run_sentiment or cli_args.run_topic
        if not analyses_requested: # Should not happen if main() checks this
             pass # No analysis requested, so technically not skippable for this reason
        elif not can_be_processed:
            print(f"  Skipping CSV row {item['row_idx']} (Doc ID: {doc_id_val}) due to missing essential data for selected analyses (Sentiment Text ('{TEXT_COLUMN_SENTIMENT}'): '{sentence_val}', Topic Context ('{TEXT_COLUMN_TOPIC}'): '{context_val}').")
            continue
        
        validated_items_for_processing.append(item)
        
    if not validated_items_for_processing:
        print(f"No processable rows found in {csv_file} after initial validation filtering (e.g. missing text/context).")
        return []

    # --- MODIFIED: Randomly sample 50 rows as per user request ---
    num_to_sample = 50
    if len(validated_items_for_processing) > num_to_sample:
        items_after_sampling = random.sample(validated_items_for_processing, num_to_sample)
        print(f"Randomly selected {num_to_sample} rows for analysis from {len(validated_items_for_processing)} valid rows.")
    else:
        items_after_sampling = validated_items_for_processing
        # No specific print here, covered by "Prepared X items for processing" later in main.

    # --- MODIFIED: Apply the --rows limit from CLI args (rows_to_process_limit) ---
    # This limit applies *after* random sampling, or to all valid rows if sampling wasn't needed/done.
    final_items_to_process = []
    if rows_to_process_limit > 0 and rows_to_process_limit < len(items_after_sampling):
        final_items_to_process = items_after_sampling[:rows_to_process_limit]
        print(f"Further limited to {rows_to_process_limit} rows due to --rows argument (applied after potential random sampling).")
    else:
        final_items_to_process = items_after_sampling
    
    if not final_items_to_process:
        print(f"No items remain after all filtering and limiting steps in preprocess_data.")

    return final_items_to_process
# --- End MODIFIED preprocess_data ---
...
def construct_sentiment_prompt(text: str) -> str:
    ...
    # This generic prompt is fine for the Finnish data too.
...
def parse_sentiment_response(api_response: str, text_snippet: str) -> str:
    ...
    # Handle common Finnish responses if model outputs them, map to English standard terms
    finnish_map = {
        "positiivinen": "Positive",
        "negatiivinen": "Negative",
        "neutraali": "Neutral"
    }
    lower_response_for_finnish_check = api_response.lower().strip()
    if lower_response_for_finnish_check in finnish_map:
        return finnish_map[lower_response_for_finnish_check]

    if cleaned_response in ["Positive", "Negative", "Neutral"]:
        return cleaned_response
    
    # Broader check for English terms if capitalization or minor variations exist
...
def parse_topic_response(api_response: str, text_snippet: str) -> str:
    ...
    cleaned_response = api_response.replace("```json", "").replace("```", "").strip()
    # Attempt to parse if it looks like JSON (sometimes models wrap output in JSON)
    try:
        # Check if it's a string that might be JSON
        if cleaned_response.startswith("{") and cleaned_response.endswith("}"):
            data = json.loads(cleaned_response)
            if isinstance(data, dict):
                # Look for common keys that might contain the topic
                for key in ["keyword", "topic", "keyphrase", "keywords", "topics"]:
                    if key in data:
                        # If it's a list, take the first element
                        topic_val = data[key]
                        if isinstance(topic_val, list) and topic_val:
                            return str(topic_val[0]).strip()
                        elif isinstance(topic_val, str):
                            return topic_val.strip()
                # If specific keys not found, but it's a dict, try to get the first value
                if data.values():
                    return str(list(data.values())[0]).strip()
        # If not JSON or parsing failed, use the cleaned response as is.
    except json.JSONDecodeError:
        pass # Fall through to use cleaned_response directly

    if not cleaned_response:
        print(f"Warning: Model returned an empty topic keyword for text: '{text_snippet[:50]}...'")
        return "Unknown"
    return cleaned_response.replace('"',"").replace("'", "").strip()


def main():
    # Global declaration must come before any references to the variable
    global CLASSIFICATION_TEMPERATURE
    ...
    parser.add_argument("--rows", type=int, default=ROWS_TO_PROCESS, help=f"Max number of rows to process from the (potentially sampled) input (0 for all). Default: {ROWS_TO_PROCESS}")
    ...
    parser.add_argument("--temperature", type=float, default=CLASSIFICATION_TEMPERATURE, help=f"Temperature for Ollama model. Default: {CLASSIFICATION_TEMPERATURE}")

    args = parser.parse_args()
    
    # Update global CLASSIFICATION_TEMPERATURE if user provided it
    CLASSIFICATION_TEMPERATURE = args.temperature

    if args.run_sentiment is None and not args.run_topic:
        args.run_sentiment = True # Default to running sentiment if no specific run flag is given
    ...
    # Sanitize input CSV filename for use in output filenames
    input_csv_filename_stem = Path(args.csv_file).stem.replace(" ", "_").replace(".", "_")

    base_filename_parts = [input_csv_filename_stem, "results"]
    ...
    print(f"Model Temperature: {CLASSIFICATION_TEMPERATURE}")
    ...
    if not items_to_process:
        print("No items to process from CSV after preprocessing and sampling/limiting. Exiting.")
        return
    
    total_rows_for_processing = len(items_to_process) # This is the count after sampling and --rows limit
    print(f"Prepared {total_rows_for_processing} items for processing.")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = [
            executor.submit(
                ...
                args.run_topic,
                args.ollama_api_url 
            ) for item in items_to_process # Use the final list from preprocess_data
        ]
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            try:
                ...
                # Use result.get('Row_Internal_Index') if available, else original CSV row_idx or just counter
                processed_row_identifier = result.get('Row_Internal_Index', result.get('row_idx', i+1))
                print(f"Completed processing for item {i+1}/{total_rows_for_processing} (Original CSV Row Index: {processed_row_identifier})")
    ...
    print(f"Finished processing all {total_rows_for_processing} items in {total_processing_duration:.2f} seconds.")

    all_results_data.sort(key=lambda x: x.get('row_idx', float('inf'))) # Sort by original CSV row index

    if all_results_data:
        final_ordered_columns = []
        first_result_keys = list(all_results_data[0].keys()) if all_results_data else []
        
        # Start with explicitly carried original columns, if they exist in results
        for col in ORIGINAL_COLUMNS_TO_CARRY:
            if col in first_result_keys:
                # Skip only context column to make the output CSV more readable
                if col not in ["context"]:
                    final_ordered_columns.append(col)
        
        # Add other original columns from the CSV that might not have been in ORIGINAL_COLUMNS_TO_CARRY explicitly
        # but were brought in because all fieldnames are copied in preprocess_data.
        # This ensures all original data from the input row is preserved.
        if items_to_process : # items_to_process contains dicts with all original keys
             original_csv_keys = items_to_process[0].keys()
             for key in original_csv_keys:
                  if key not in final_ordered_columns and key != 'row_idx' and key not in ["context"]: # row_idx is internal, Row_Internal_Index is preferred
                       final_ordered_columns.append(key)


        # Add specific generated columns in a sensible order
        # "Row_Internal_Index" is same as "row_idx" from input item, which is original CSV 1-based index.
        if "Row_Internal_Index" in first_result_keys and "Row_Internal_Index" not in final_ordered_columns :
             final_ordered_columns.append("Row_Internal_Index")
        elif "row_idx" in first_result_keys and "row_idx" not in final_ordered_columns: # Fallback if only row_idx is there
             final_ordered_columns.append("row_idx")


        if "Sentence_Snippet_Generated" in first_result_keys and "Sentence_Snippet_Generated" not in final_ordered_columns:
        ...
        if args.run_sentiment and args.sentiment_model:
            ...
            sent_col = f"Sentiment ({sm_name_formatted})"
            sent_corr_col = f"{sent_col} Correct"
            if sent_col in first_result_keys and sent_col not in final_ordered_columns: final_ordered_columns.append(sent_col)
            if sent_corr_col in first_result_keys and sent_corr_col not in final_ordered_columns: final_ordered_columns.append(sent_corr_col)
            if "Sentiment_Processing_Time_s" in first_result_keys and "Sentiment_Processing_Time_s" not in final_ordered_columns : final_ordered_columns.append("Sentiment_Processing_Time_s")
        
        if args.run_topic and args.topic_model:
            tm_name_formatted = args.topic_model.replace(':', '_')
            topic_col = f"Topic Keyword ({tm_name_formatted})"
            if topic_col in first_result_keys and topic_col not in final_ordered_columns: final_ordered_columns.append(topic_col)
            if "Topic_Processing_Time_s" in first_result_keys and "Topic_Processing_Time_s" not in final_ordered_columns: final_ordered_columns.append("Topic_Processing_Time_s")
        
        if "Processing_Error_Details" in first_result_keys and "Processing_Error_Details" not in final_ordered_columns:
            final_ordered_columns.append("Processing_Error_Details")

        all_actual_keys_in_results = set()
        for res_dict in all_results_data:
            all_actual_keys_in_results.update(res_dict.keys())
        
        for key in sorted(list(all_actual_keys_in_results)):
            if key not in final_ordered_columns:
                final_ordered_columns.append(key) # Add any remaining keys
        
        final_ordered_columns = list(dict.fromkeys(final_ordered_columns)) # Ensure unique columns
        ...
        try:
            ...
            # Varmistetaan että context-sarake on olemassa ennen sen poistamista
            columns_to_remove = ["context"]
            for col in columns_to_remove:
                if col in df_results.columns:
                    df_results = df_results.drop(columns=[col])
                    # Poista context myös final_ordered_columns-listasta, jos se on siellä
                    if col in existing_final_cols:
                        existing_final_cols.remove(col)
            
            df_results = df_results[existing_final_cols] # Reorder and select
            df_results.to_csv(output_csv_path, index=False, encoding='utf-8')
            print(f"\nResults successfully saved to {output_csv_path}")
        except Exception as e:
            print(f"\nError saving results to CSV with ordered columns: {e}. Columns attempted: {final_ordered_columns}")
            ...
            try:
                # Even in fallback, ensure only context is removed
                df_fallback = pd.DataFrame(all_results_data)
                for col in columns_to_remove:
                    if col in df_fallback.columns:
                        df_fallback = df_fallback.drop(columns=[col])
                df_fallback.to_csv(output_csv_path, index=False, encoding='utf-8')
        ...
        print("No results data was generated to save to CSV.")


    md_content = f"# Analysis Results ({run_start_datetime.strftime('%Y-%m-%d %H:%M:%S')})\n\n"
    md_content += f"- Input CSV: `{args.csv_file}`\n"
    md_content += f"- Processed Rows: {total_rows_for_processing} (after sampling/limiting from initially prepared items)\n"
    ...
    md_content += f"- Ollama API URL: `{args.ollama_api_url}`\n"
    md_content += f"- Model Temperature: `{CLASSIFICATION_TEMPERATURE}`\n"
    md_content += f"- Output CSV: `{output_csv_path.name}` (in dir: `{output_path}`)\n"
    md_content += f"- Output Markdown: `{output_md_path.name}` (in dir: `{output_path}`)\n\n"
    ...
    if args.run_sentiment and all_results_data:
        ...
        if all_results_data and sentiment_model_col_name in all_results_data[0]:
            # Pass GROUND_TRUTH_COLUMN explicitly as it might be the dummy "NOT_APPLICABLE_..."
            ...
            if metrics_summary:
                ...
                acc_display = f"{metrics_summary['accuracy']:.4f}" if isinstance(metrics_summary['accuracy'], float) else metrics_summary['accuracy']
                md_content += f"- Accuracy: {acc_display} ({metrics_summary['correct_predictions']}/{metrics_summary['valid_predictions']} valid predictions with ground truth)\n"
                if metrics_summary['valid_predictions'] == 0:
                     md_content += "  (No ground truth was available or parsable for metric calculation)\n"
                for cls, cls_metrics in metrics_summary.get('class_metrics', {}).items():
            ...
            md_content += f"## Sentiment Analysis Metrics for `{args.sentiment_model}`\n\n"
            md_content += f"- Metrics could not be calculated. Sentiment column '{sentiment_model_col_name}' not found or no results processed.\n\n"

    if args.run_topic and all_results_data: # Added check for all_results_data
        ...
        md_content += f"## Topic Modeling Results for `{args.topic_model}`\n\n"
        md_content += "No topic modeling results to display.\n\n"


    if all_results_data: # Only show table if there's data
        md_content += "## Detailed Results Preview (Sample - first 20 rows)\n\n"
        # DOC_ID_COLUMN is "ID", GROUND_TRUTH_COLUMN is "NOT_APPLICABLE_..."
        table_header = [DOC_ID_COLUMN] 
        # Only add Ground Truth to header if it's not the dummy one, or if we want to show it's N/A
        # For this script, it will always be the dummy, so we can choose to show it or not.
        # Let's show it to be clear that it was considered.
        table_header.append(GROUND_TRUTH_COLUMN)
        ...
        for i, res in enumerate(all_results_data[:20]):
            row_values = [str(res.get(DOC_ID_COLUMN, "N/A"))]
            row_values.append(str(res.get(GROUND_TRUTH_COLUMN, "N/A"))) # Will show "N/A" or "" generally

            if args.run_sentiment:
                sm_name_formatted = args.sentiment_model.replace(':', '_')
                row_values.append(str(res.get(f"Sentiment ({sm_name_formatted})", "N/A")))
                row_values.append(str(res.get(f"Sentiment ({sm_name_formatted}) Correct", "N/A"))) # Will be N/A mostly
        ...
        md_content += "## Detailed Results Preview\n\nNo detailed results to display as no data was processed.\n"


    try:
        with open(output_md_path, 'w', encoding='utf-8') as f_md:
            f_md.write(md_content)
        print(f"\nMarkdown report successfully saved to {output_md_path}")
    except Exception as e:
