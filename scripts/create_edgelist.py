...
if __name__ == '__main__':
    ...
    mapping = {k.lower():v for k,v in zip(df.get_column("name_short"),df.get_column("ISO2"))} | {"usa": "us", "holland": "nl"}
    for f in sorted(glob("../ParlaMint_entities_filtered/**.parquet")):
        iso2_cc = coco.convert(f.split("/")[-1].split("_")[0], to="iso2")
        ...
        ).sink_parquet(f"{out_dir}/{iso2_cc}_edges.parquet", compression="zstd")
